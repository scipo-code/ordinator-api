...
pub fn read_csv_files() {
    ...
    let activities: Vec<Activity> =
        read_csv::<Activity>(input_opr_path).expect("Failed to read CSV file");
    let time_postings: Vec<TimePosting> =
        read_csv::<TimePosting>(input_time_postings_path).expect("Failed to read CSV file");
...
fn join_csv_files(jobs: Vec<Job>, activities: Vec<Activity>, time_postings: Vec<TimePosting>) {
    let activities_by_order: HashMap<String, Vec<Activity>> =
        activities.iter().fold(HashMap::new(), |mut acc, activity| {
            acc.entry(activity.order.clone().unwrap())
    ...
    let time_postings_by_activity: HashMap<String, TimePosting> =
        time_postings
            .into_iter()
            .fold(HashMap::new(), |mut acc, time_posting| {
    ...
    if row == 0 {
        ...
        for header in header_set {
            workbook
                .get_worksheet("scheduler_data")
                .unwrap()
                .unwrap()
                .write_string(row, i, header, None)
                .unwrap();
            i += 1;
        }
    } else {
        for job in &jobs {
            if let Some(related_activities) = activities_by_order.get(&job.order.clone().unwrap()) {
                for activity in related_activities {
                    if let Some(time_posting) =
                        time_postings_by_activity.get(&activity.activity.clone().unwrap())
                    {
...
fn export_to_excel() -> Workbook {
    ...
    workbook
        .add_worksheet(Some("scheduler_data"))
