...
def strategic_plotting(args, json_data):
    ...
    df['strategic_objective_value'] = df['strategic_objective_value'].astype(float)

    # Plotting
    plt.figure(figsize=(10, 5))
    if args.relative:
        df['rel_time'] = (df['timestamp'] - df['timestamp'].iloc[0]).dt.seconds
        plt.plot(df['rel_time'][args.offset:], (df['strategic_objective_value'][args.offset:]), linestyle='-', antialiased=True)
    else:
        plt.plot(df['timestamp'][args.offset:], (df['strategic_objective_value'][args.offset:]), linestyle='-', antialiased=True)
