...
// 
// }
// One thing is for sure. If you decide to do this over, you should do it correctly. The issue is that you will have so many bugs
// due to the fact that many generic things have to be changed in 4 different places, even though you want the same behavior. I
// think that making the behavior generic is the most important thing here.
impl Algorithm<StrategicSolution, StrategicParameters, PriorityQueue<WorkOrderNumber, u64>> {
    pub fn update_the_locked_in_period(&mut self, work_order_number: &WorkOrderNumber, locked_in_period: &Period) -> Result<()> {
        self.solution.strategic_scheduled_work_orders.insert(*work_order_number, Some(locked_in_period.clone()));

        let strategic_parameter = self.parameters.strategic_work_order_parameters.get_mut(work_order_number).with_context(|| format!("{:?} not found in {}", work_order_number, std::any::type_name::<StrategicParameters>()))?;
    ...
    //         unsafe {
    //             let scheduled_work_order_1 = self.solution.strategic_periods.get_mut(randomly_chosen[0]).unwrap() as *mut Option<Period>;
    //             let scheduled_work_order_2 = self.solution.strategic_periods.get_mut(randomly_chosen[1]).unwrap() as *mut Option<Period>;
    ...
    fn determine_urgency(&mut self, strategic_objective_value: &mut StrategicObjectiveValue) -> Result<()> {
        for (work_order_number, scheduled_period) in &self.solution.strategic_scheduled_work_orders {
            let optimized_period = match scheduled_period {
                ...
                None => {
                    self.parameters.strategic_periods.last().context("There should always be a last .parameters.eriod")?
                }
            };
...
impl Algorithm<StrategicSolution, StrategicParameters, PriorityQueue<WorkOrderNumber, u64>> {
    ...
    pub fn schedule_forced_work_order(&mut self, force_schedule_work_order: &ForcedWorkOrder) -> Result<()> {
        ...
        self.update_the_locked_in_period(force_schedule_work_order.work_order_number(), &locked_in_period.clone())
    ...
    pub fn determine_best_permutation(&self, work_load: HashMap<Resources, Work>, period: &Period, schedule: ScheduleWorkOrder) -> Result<Option<StrategicResources>> {
        ...
        for  _ in 0..10 {
            let mut technician_permutation = difference_resources.clone().into_values().collect::<Vec<_>>();
            ...
            work_load_permutation.shuffle(&mut rng);
                ...
                let strategic_resource_loadings_option = match schedule {
                    ...
                    ScheduleWorkOrder::Unschedule => {
                        let order_map: HashMap<_, _> = technician_permutation.clone().into_iter().enumerate().map(|(i, v)| (v.id, i)).collect();

                        let strategic_resources_loading_vec: Vec<_> = strategic_resources_loading.clone().into_iter().sorted_by_key(|ele| order_map.get(&ele.0)).map(|ele| ele.1).collect();

                        let strategic_resources_option = determine_unschedule_work_resource_loadings(period, &strategic_resources_loading_vec, &mut work_load_permutation);
                ...
                // be replenished
                // There are many different cases here. We should work on the one associated with the Unschedule
                // which means that if the unscheduling operations is not possible we should try to make something.
...
fn determine_unschedule_work_resource_loadings(period: &Period, loading_resources: &[OperationalResource], work_load_permutation: &mut [(Resources, Work)]) -> Option<StrategicResources> {
    ...
    for (resources, work) in work_load_permutation.iter_mut() {
        ...
        // Here we do not want to go over the operational_resource in random order. We want to determine the best possible
        // way to visit the structure to make a correct scheduling approach.
        //
        // 
        for operational_resource in loading_resources_cloned.iter_mut() {
            ...
            if operational_resource.total_hours >= *work {
                ...
                strategic_resources
                    .update_load(period, *resources, *work, operational_resource, LoadOperation::Sub);
    ...
    if work_load_permutation.iter().all(|ele| ele.1 == Work::from(0.0)) {
...
/// This function determines the resource load for when a work order should be forced into the
/// schedule. 
fn determine_forced_work_order_resource_loadings(
    ...
    for (resources, work) in work_load_permutation.iter_mut() {
        let mut qualified_technicians = technician_permutation
            .iter_mut()
            .filter(|or| {
                or.skill_hours.keys().contains(resources)
            })
            .collect::<Vec<_>>();
        ...
        let work_load_by_resources_by_technician = work.divide_work(Work::from(qualified_technicians.len() as f64));

        for operational_resource in qualified_technicians {
            operational_resource.total_hours -= work_load_by_resources_by_technician;
            operational_resource.skill_hours.iter_mut().for_each(|(_, wor)| *wor -= work_load_by_resources_by_technician);
            work_order_resource_loadings.update_load(period, *resources, work_load_by_resources_by_technician, operational_resource, LoadOperation::Add);
        }
    }
...
fn determine_normal_work_order_resource_loadings(
    ...
    for operation_load in work_load_permutation {
        for operational_resource in technician_permutation.iter_mut() {
            if !operational_resource.skill_hours.keys().contains(&operation_load.0) {
                continue;
            }

            if operation_load.1 <= operational_resource.total_hours {
                operational_resource.skill_hours.iter_mut().for_each(|(_, wor)| *wor -= operation_load.1);  
                operational_resource.total_hours -= operation_load.1;
                work_order_resource_loadings.update_load(period,operation_load.0, operation_load.1, operational_resource, LoadOperation::Add);
                operation_load.1 = Work::from(0.0);
                break;
            } else {
                operation_load.1 -= operational_resource.total_hours;
                operational_resource.skill_hours.iter_mut().for_each(|(_res, wor)| *wor = Work::from(0.0));
                operational_resource.total_hours = Work::from(0.0);
                work_order_resource_loadings.update_load(period, operation_load.0, operational_resource.total_hours, operational_resource, LoadOperation::Add);
            }
        }
...
impl ActorBasedLargeNeighborhoodSearch for Algorithm<StrategicSolution, StrategicParameters, PriorityQueue<WorkOrderNumber, u64>> {
    ...
    fn calculate_objective_value(&mut self) -> Result<ObjectiveValueType<Self::ObjectiveValue>> {
        ...
        self.determine_urgency(&mut strategic_objective_value).context("could not determine strategic urgency")?;
    ...
    // into the priority queue and then the locked_in_period constaint in simply handled in there.
    ...
    fn schedule(&mut self) -> Result<()> {
        ...
        // we instead should strive to make a clean schedule function. and then we should
        // make a shared_state_update function.
        // Maybe it is also time to clean up in all this code.
    ...
    fn unschedule(
        ...
        for work_order_number in sampled_work_order_keys {
            ...
            let weight = self.parameters.strategic_work_order_parameters.get(work_order_number).context("Parameters should always be available")?.weight;
            self.solution_intermediate.push(*work_order_number, weight);
        }
        Ok(())
    }
    ...
    ) -> Result<StrategicResponseResources> 
    {
        match strategic_resources_request {
            ...
            StrategicRequestResource::GetPercentageLoadings { periods_end:_, resources: _ } => {
                ...
                Algorithm::assert_that_capacity_is_respected(loadings, capacities).context("Loadings exceed the capacities")?;
    ...
    ) -> Result<StrategicResponseScheduling>
{
        match strategic_scheduling_request {
            StrategicRequestScheduling::Schedule(schedule_work_order) => {
                ...
                for work_order_number in schedule_work_order.work_order_number {
                    let strategic_parameter = self.parameters.strategic_work_order_parameters.get_mut(&work_order_number).unwrap();
                    if strategic_parameter.excluded_periods.contains(&period) {
            ...
            StrategicRequestScheduling::ExcludeFromPeriod(exclude_from_period) => {
                ...
                let period = self.parameters.strategic_periods.iter().find(|period| {
                    period.period_string() == exclude_from_period.period_string().clone()
                }).with_context(|| format!("{} was not found in the {}", exclude_from_period.period_string, std::any::type_name::<TimeEnvironment>()))?;
                ...
                for work_order_number in exclude_from_period.work_order_number {
                    ...
                    // assert!(!strategic_parameter.excluded_periods.contains(self.solution.strategic_periods.get(&work_order_number).as_ref().unwrap().as_ref().unwrap()));
                    ...
                    self.solution.strategic_scheduled_work_orders.insert(work_order_number, last_period);
    ...
    fn unschedule_specific_work_order(&mut self, work_order_number: WorkOrderNumber) -> Result<()> {
        ...
        if let Some(unschedule_from_period) = unschedule_from_period {
            let strategic_parameter = self.parameters.strategic_work_order_parameters.get(&work_order_number).unwrap();
    ...
    pub fn populate_priority_queue(&mut self) {
        for work_order_number in self.solution.strategic_scheduled_work_orders.keys() {
            ...
            if self.solution.strategic_scheduled_work_orders.get(work_order_number).unwrap().is_none() {
...
mod tests {
    ...
    use crate::agents::{strategic_agent::algorithm::{
        ...
        }, AlgorithmUtils, ArcSwapSharedSolution, SharedSolution, TacticalSolutionBuilder, WhereIsWorkOrder};
    ...
    fn test_update_load_1() {
        ...
        let operational_resources_by_period = vec![(operational_id.to_string(), operational_resource.clone())].into_iter().collect();

        let strategic_resources_inner:  HashMap<Period, HashMap<String, OperationalResource>> = vec![(period.clone(), operational_resources_by_period)].into_iter().collect();
        

        let mut strategic_resources = StrategicResources::new(strategic_resources_inner);

        strategic_resources.update_load(&period, resource, load, &operational_resource, LoadOperation::Add);

        assert_eq!(strategic_resources.0.get(&period).unwrap().get(operational_id).unwrap().total_hours, Work::from(130.0));
        assert!(strategic_resources.0.get(&period).unwrap().get(operational_id).unwrap().skill_hours.values().all(|wor| *wor == Work::from(130.0)));
    }
    ...
    fn test_update_load_2() {
        ...
        assert_eq!(strategic_resources.0.get(&period).unwrap().get(operational_id).unwrap().total_hours, Work::from(130.0));
        assert!(strategic_resources.0.get(&period).unwrap().get(operational_id).unwrap().skill_hours.values().all(|wor| *wor == Work::from(130.0)));
        assert!(strategic_resources.0.get(&period).unwrap().get(operational_id).unwrap().skill_hours.contains_key(&resource));
    }
    #[test]
    fn test_update_load_3() {
        ...
        strategic_resources.update_load(&period, resource, load, &operational_resource, LoadOperation::Sub);

        assert_eq!(strategic_resources.0.get(&period).unwrap().get(operational_id).unwrap().total_hours, Work::from(70.0));
        assert!(strategic_resources.0.get(&period).unwrap().get(operational_id).unwrap().skill_hours.values().all(|wor| *wor == Work::from(70.0)));
        assert!(strategic_resources.0.get(&period).unwrap().get(operational_id).unwrap().skill_hours.contains_key(&resource));
    }
    ...
    fn test_determine_normal_work_order_resource_loadings_1() {
        ...
        let mut technician_permutation = vec![
            OperationalResource::new("OP_TEST_0", Work::from(40.0), vec![Resources::MtnMech, Resources::MtnElec]),
            OperationalResource::new("OP_TEST_1", Work::from(40.0), vec![Resources::MtnScaf, Resources::MtnElec]),
        ];
        ...
        let strategic_resource_option = super::determine_normal_work_order_resource_loadings(&period, &mut technician_permutation, &mut work_load_permutation);
    ...
    fn test_determine_normal_work_order_resource_loadings_2() {
        ...
        let operational_resource_1 = OperationalResource::new("OP_TEST_0", Work::from(40.0), vec![Resources::MtnMech, Resources::MtnElec]);
    ...
    fn test_determine_forced_work_order_resource_loadings_1() {
        ...
        let operational_resource_1 = OperationalResource::new("OP_TEST_0", Work::from(45.0), vec![Resources::MtnMech, Resources::MtnElec]);
        let operational_resource_2 = OperationalResource::new("OP_TEST_1", Work::from(45.0), vec![Resources::MtnScaf, Resources::MtnElec]);
        let mut strategic_resources = StrategicResources::default();
    ...
    fn test_determine_forced_work_order_resource_loadings_2() {
        ...
        let operational_resource_1 = OperationalResource::new("OP_TEST_0", Work::from(30.0), vec![Resources::MtnMech, Resources::MtnElec]);
        let operational_resource_2 = OperationalResource::new("OP_TEST_1", Work::from(30.0), vec![Resources::MtnScaf, Resources::MtnElec]);
        let mut strategic_resources = StrategicResources::default();
    ...
    fn test_determine_forced_work_order_resource_loadings_3() {
        ...
        let operational_resource_1 = OperationalResource::new("OP_TEST_0", Work::from(40.0), vec![Resources::MtnMech, Resources::MtnElec, Resources::VenMech]);
        let operational_resource_2 = OperationalResource::new("OP_TEST_1", Work::from(40.0), vec![Resources::MtnScaf, Resources::MtnElec, Resources::VenMech]);
        let mut strategic_resources = StrategicResources::default();
    ...
    fn test_determine_unschedule_work_order_loadings_1() {
        ...
        let loading_resources = [
            OperationalResource::new("OP_TEST_0", Work::from(20.0), vec![Resources::MtnMech, Resources::MtnElec]),
            OperationalResource::new("OP_TEST_1", Work::from(20.0), vec![Resources::MtnScaf, Resources::MtnElec]),
        ];
    ...
    fn test_determine_unschedule_work_order_loadings_2() -> Result<()> {
        ...
        let loading_resources = [
            OperationalResource::new("OP_TEST_1", Work::from(6.0), vec![Resources::MtnMech, Resources::Prodtech, Resources::MtnElec]),
            OperationalResource::new("OP_TEST_2", Work::from(0.0), vec![Resources::MtnScaf, Resources::MtnRigg, Resources::MtnLagg]),
            OperationalResource::new("OP_TEST_0", Work::from(2.0), vec![Resources::MtnInst, Resources::MtnMech, Resources::MtnElec]),
        ];
        
        // We should readjust this to take in a Vec<T> instead of a HashMap<K, V>. That is a much better approach
        // for dealing with this. And then we reuse the structure that we already have.
        // QUESTION:
    ...
    fn test_unschedule_random_work_orders() -> Result<()> {
        ...
        let operational_resource_0 = OperationalResource::new("OP_TEST_0", Work::from(40.0), vec![Resources::MtnMech, Resources::MtnElec]);
        let operational_resource_1 = OperationalResource::new("OP_TEST_1", Work::from(40.0), vec![Resources::MtnMech, Resources::MtnElec]);
        ...
        let mut strategic_parameters = StrategicParameters::new(&Asset::Unknown, scheduling_environment.lock().unwrap())?;
        ...
        let strategic_parameters = StrategicParameters::new(&Asset::Unknown, scheduling_environment.lock().unwrap())?;
        ...
        strategic_algorithm.solution.strategic_scheduled_work_orders.insert(work_order_number_1, Some(periods[0].clone()));
        strategic_algorithm.solution.strategic_scheduled_work_orders.insert(work_order_number_2, Some(periods[1].clone()));
        strategic_algorithm.solution.strategic_scheduled_work_orders.insert(work_order_number_3, Some(periods[1].clone()));

        let operational_resource_0 = OperationalResource::new("OP_TEST_0", Work::from(30.0), vec![Resources::MtnMech, Resources::MtnElec, Resources::Prodtech]);
        let operational_resource_1 = OperationalResource::new("OP_TEST_1", Work::from(150.0), vec![Resources::MtnMech, Resources::MtnElec, Resources::Prodtech]);

        strategic_algorithm.solution.strategic_loadings.insert_operational_resource(periods[0].clone(), operational_resource_0);
        strategic_algorithm.solution.strategic_loadings.insert_operational_resource(periods[1].clone(), operational_resource_1);
        ...
        strategic_algorithm
            .unschedule(&mut StrategicOptions { number_of_removed_work_order: 2, rng })
    ...
    fn test_unschedule_work_order_none_in_scheduled_period() -> Result<()> {
        ...
        let strategic_parameter = StrategicParameter::new(
            ...
            1000,
            HashMap::from([
                (Resources::MtnMech, Work::from(5.0))
            ]),
        );
